...
namespace sed_pipeline {
  ...
  auto parse_subst(std::string_view expr,
    ...
    for (; i < expr.size(); ++i) {
      char f = expr[i];
      if (f == 'g') g = true;
      else if (f == 'p') pflag = true;
      else if (f == ' ') continue;
      else return std::unexpected("unknown flag in s command");
    }
  ...
  auto parse_y_cmd(std::string_view line) -> cp::Result<Script> {
    ...
    s.has_ymap = true;
    for (size_t k = 0; k < s.ymap.size(); ++k) s.ymap[k] = static_cast<unsigned char>(k);
  ...
  auto parse_script_line(std::string_view line,
    ...
    for (const auto& part : split_commands(line)) {
      size_t i = 0;
      while (i < part.size() &&
             std::isspace(static_cast<unsigned char>(part[i]))) ++i;
      ...
      cp::Result<Script> s;
      if (!cmd.empty() && cmd[0] == 's') s = parse_subst(cmd, syntax);
      else if (!cmd.empty() && cmd[0] == 'y') s = parse_y_cmd(cmd);
      else s = parse_simple_cmd(cmd);
  ...
  auto read_script_file(const std::string& path,
    ...
    if (!in.is_open()) return std::unexpected("cannot open script file '" + path + "'");
    std::vector<Script> out;
    std::string line;
  ...
  auto build_config(const CommandContext<SED_OPTIONS.size()>& ctx)
    ...
    Config cfg;
    cfg.suppress_output =
        ctx.get<bool>("--quiet", false) || ctx.get<bool>("-n", false) ||
      ...
      switch (s.kind) {
        case Script::Kind::Subst: {
          if (s.has_ymap) {
            ...
            std::string repl = s.ecma_repl ? to_ecma_repl(s.replacement)
                                           : s.replacement;
            std::string replaced =
