...
- Rechecked GNU cpio `src/main.c`/archive flow, procps-ng `free.c`/`top.c`/`watch.c`, lsof `lib/lsof.c`, upstream tree `tree.c`, GNU coreutils `uptime.c`, and man-db `src/man.c` for the remaining easy-to-isolate P2 command batch. Added local-only Release shape/performance probes for `cpio -o`, `free -m`, `lsof --no-headers -F -t 50`, `man --list`, capped-prefix `top -b --rows 8`, `tree -L 1 tree`, default `uptime`, and finite `watch -n 0 -c 1 -t printf watch-ok`. These probes record Winux timing and output shape without pretending a stable GNU/MSYS reference exists for machine-dependent Windows state.
- Refreshed `DOCS/generated/command_performance_baseline.*`: the current Release baseline has 240/240 passing probes, no probe failures, and no tracked probe at or above 5x slower than reference. Refreshed `DOCS/generated/command_compatibility_matrix.*`: every non-local command now has upstream source LOC mapping, unit-test count, and performance-probe count; `tzset` remains the only local-family command. Follow-up implementation depth for `cpio`, `free`, `lsof`, `man`, `top`, `tree`, `uptime`, and `watch` is tracked in GitHub issue #114 rather than this audit document.
Major gaps:
