...
| `xargs` | ✅ Done | High | Build and execute command lines from input | `-n, --max-args=MAX-ARGS`: Use at most MAX-ARGS arguments per command line<br>`-I, --replace[=R-STR]`: Replace R-STR in initial-arguments with names read from standard input<br>`-P, --max-procs=MAX-PROCS`: Run up to MAX-PROCS processes at a time<br>`-t, --verbose`: Print the command line on the standard error output before executing it<br>`-0, --null`: Input items are terminated by a null character instead of by whitespace<br>`-d, --delimiter=DELIM`: Input items are terminated by DELIM instead of by whitespace [NOT SUPPORT] | Command execution from input, SmallVector optimization |
