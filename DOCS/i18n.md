...
descriptions. Missing keys remain in English. Runtime diagnostics use the same
`messages` map with stable keys such as `core.error.command_not_found` and
`utils.file.error.open`.
...
`scripts/i18n_scan.py` inventories output calls and `std::unexpected` error
construction without modifying source files:

```sh
python scripts/i18n_scan.py --output i18n-scan.json
```

The scanner deliberately marks data-bearing and legacy output for review; it
does not assume that every string passed to an output function is translatable.
